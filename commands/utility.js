...
const utilityCommands = {
        ...
        execute: async (context) => {
            ...
            try {
                ...
                try {
                    ...
                    await bot.sendMessage(chatId, '⚠️ Could not fetch a quote right now. Please try again later.');
                }
            }
        }
    },
    ...
    lyrics: {
        ...
        adminOnly: false,
        execute: async (context) => {
            const { chatId, bot, args } = context;

            if (!args.length) {
                return bot.sendMessage(chatId, "❌ Please provide a song title.\nExample: `lyrics despacito`");
            }
            ...
            try {
                ...
                if (data && data.lyrics) {
                    ...
                    await bot.sendMessage(chatId, lyricsMsg);
                    return;
                }
                ...
                try {
                    ...
                    if (result && result.lyrics) {
                        ...
                        await bot.sendMessage(chatId, "❌ Couldn't fetch lyrics, try again later!");
                    }
                } catch (fallbackError) {
                    console.error("Fallback lyrics API failed:", fallbackError.message);
                    await bot.sendMessage(chatId, "⚠️ Both lyrics sources are down, please try again later.");
                }
            }
        }
    },
        ...
        execute: async (context) => {
            ...
            try {
                ...
                if (!cited && !directImage) {
                    return await bot.sendMessage(chatId, '❌ Please reply to an image (or sticker/document image) with `url`');
                }
                ...
                try {
                    ...
                    if (catboxRes.data && String(catboxRes.data).startsWith('http')) {
                        return await bot.sendMessage(chatId, `✅ Uploaded to Catbox:\n${catboxRes.data}`);
                    }
                ...
                try {
                    ...
                    if (url) {
                        return await bot.sendMessage(chatId, `✅ Uploaded to ImgBB:\n${url}`);
                    }

                    throw new Error('ImgBB did not return a URL');
                } catch (imgErr) {
                    console.error('ImgBB upload failed:', imgErr.message || imgErr);
                    return await bot.sendMessage(chatId, '⚠️ Failed to upload image to both Catbox and ImgBB. Please try again.');
                }

            } catch (err) {
                console.error('Image upload error:', err && (err.stack || err.message || err));
                return await bot.sendMessage(chatId, '❌ Error processing the image. Make sure you replied to an image.');
            }
        }
    },
    ...
    tiny: {
        ...
        execute: async (context) => {
            ...
            if (!args[0]) {
                return await bot.sendMessage(chatId, '❌ Please provide a URL to shorten.');
            }
            ...
            try {
                ...
                try {
                    ...
                    return await bot.sendMessage(chatId, '⚠️ Failed to shorten the link. Please try again later.');
                }
            }
