...
const downloaderCommands = {
    play: {
        ...
        execute: async (context) => {
            ...
            if (!searchQuery) {
                return sock.sendMessage(chatId, { text: "❌ Please provide a YouTube link or song name." }, {quoted: message});
            }
    ...
    instagram: {
        ...
        execute: async (context) => {
            ...
            if (!url) {
                return sock.sendMessage(chatId, { text: '❌ Please provide a valid Instagram link.' }, { quoted: message });
            }
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: '⚠️ Could not fetch Instagram media. Please try again later.' }, { quoted: message });
            }
        }
    },
        ...
        execute: async (context) => {
            ...
            try {
                ...
                if (!data?.success || !data?.results || data.results.length === 0) {
                    return await sock.sendMessage(chatId, { text: "⚠️ No images found." }, { quoted: message });
                }
                ...
                // Send image
                const imageBuffer = await (await axios.get(randomImage, { responseType: "arraybuffer" })).data;
                await sock.sendMessage(
                    chatId,
                    {
                        image: imageBuffer,
        ...
        execute: async (context) => {
            ...
            try {
                ...
                if (response.data && response.data.url) {
                    const imageBuffer = await (await axios.get(response.data.url, { responseType: "arraybuffer" })).data;
                    await sock.sendMessage(chatId, {
                        image: imageBuffer,
                        caption: "> 𝚉𝙴𝙽-𝙼𝙳 \n> _waifu.pics_"
                    }, { quoted: message });
                ...
                try {
                    ...
                    if (fallback.data && fallback.data.result) {
                        const imageBuffer = await (await axios.get(fallback.data.result, { responseType: "arraybuffer" })).data;
                        await sock.sendMessage(chatId, {
                            image: imageBuffer,
                            caption: "> 𝚉𝙴𝙽-𝙼𝙳"
                        }, { quoted: message });
                    } else {
                        await sock.sendMessage(chatId, { text: "❌ Couldn't fetch waifu picture, try again later!" }, { quoted: message });
                    }
                } catch (fallbackError) {
                    console.error("Fallback waifu API failed:", fallbackError.message);
                    await sock.sendMessage(chatId, { text: "⚠️ Both waifu sources are down, please try again later." }, { quoted: message });
                }
            }
        }
    },
