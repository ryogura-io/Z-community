...
const groupCommands = {
    add: {
        ...
        execute: async (context) => {
            ...
            if (args.length === 0) {
                await sock.sendMessage(chatId, { text: '❌ Please provide a phone number.\nUsage: !add 1234567890' }, { quoted: message });
                return;
            }
            ...
            try {
                ...
                if (result[0].status === 'success') {
                    await sock.sendMessage(chatId, { text: `✅ Successfully added ${phoneNumber} to the group.` }, { quoted: message });
                } else {
                    await sock.sendMessage(chatId, { text: `❌ Failed to add ${phoneNumber}. They might have privacy settings enabled.` }, { quoted: message });
                }
            } catch (error) {
                await sock.sendMessage(chatId, { text: '❌ Error adding member to group.' }, { quoted: message });
            }
        }
    },
    ...
    remove: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can remove members.' }, { quoted: message });
                return;
            }
            ...
            if (!targetUser) {
                await sock.sendMessage(chatId, { text: '❌ Please mention a user or reply to their message.' }, { quoted: message });
                return;
            }
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: '❌ Error removing member from group.' }, { quoted: message });
            }
        }
    },
    ...
    promote: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can promote members.' }, { quoted: message });
                return;
            }
    ...
    demote: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can demote members.' }, { quoted: message });
                return;
            }
    ...
    groupinfo: {
        ...
        execute: async (context) => {
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: '❌ Error getting group information.' }, { quoted: message });
            }
        }
    },
    ...
    link: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can fetch the group link.' }, { quoted: message });
                return;
            }
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: `🔗 Group Invite Link:\n${inviteLink}` }, { quoted: message });
            } catch (error) {
                console.error('Error fetching group link:', error);
                await sock.sendMessage(chatId, { text: '❌ Error fetching group link.' }, { quoted: message });
            }
        }
    },
    ...
    close: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can close the group.' }, { quoted: message });
                return;
            }
    ...
    open: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can open the group.' }, { quoted: message });
                return;
            }
            
            try {
                await sock.groupSettingUpdate(chatId, 'not_announcement');
                await sock.sendMessage(chatId, { text: '🔓 Group opened. All members can send messages now.' }, { quoted: message });
            } catch (error) {
    ...
    tag: {
        ...
        execute: async (context) => {
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: '❌ Error tagging members.' }, { quoted: message });
            }
        }
    },
    ...
    tagall: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can tag all members.' }, { quoted: message });
                return;
            }
            ...
            try {
                ...
                await sock.sendMessage(chatId, { text: '❌ Error tagging all members.' }, { quoted: message });
            }
        }
    },
    ...
    resetlink: {
        ...
        execute: async (context) => {
            ...
            const isGroupAdmin = await permissions.isGroupAdmin(sender, chatId, sock);
            if (!isGroupAdmin) {
                await sock.sendMessage(chatId, { text: '❌ Only group admins can reset the group link.' }, { quoted: message });
                return;
            }
            
            try {
                await sock.groupRevokeInvite(chatId);
                await sock.sendMessage(chatId, { text: '🔗 Group invite link has been reset. Previous links are no longer valid.' }, { quoted: message });
            } catch (error) {
                await sock.sendMessage(chatId, { text: '❌ Error resetting group link.' }, { quoted: message });
            }
        }
    }
};
