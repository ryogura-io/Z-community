...
const mediaCommands = {
    view: {
        ...
        execute: async (context) => {
            ...
            if (!quotedMessage) {
                await sock.sendMessage(chatId, { text: '❌ Please reply to a message containing media.' }, { quoted: message });
                return;
            }
            ...
            try {
                ...
                if (!hasMedia) {
                    await sock.sendMessage(chatId, { text: '❌ The replied message does not contain any media.' }, { quoted: message });
                    return;
                }
                
                await sock.sendMessage(chatId, { text: '⏳ Downloading media...' }, { quoted: message });
                ...
                if (buffer.length > config.get('mediaDownloadLimit')) {
                    await sock.sendMessage(chatId, { text: '❌ Media file is too large to download.' }, { quoted: message });
                    return;
                }
                
                // Determine media type and send
                if (quotedMessage.imageMessage) {
                    await sock.sendMessage(chatId, { image: buffer, caption: '📥 Downloaded Image' }, { quoted: message });
                } else if (quotedMessage.videoMessage) {
                    await sock.sendMessage(chatId, { video: buffer, caption: '📥 Downloaded Video' }, { quoted: message });
                } else if (quotedMessage.audioMessage) {
                    await sock.sendMessage(chatId, { audio: buffer, mimetype: 'audio/mp4' }, { quoted: message });
                } else if (quotedMessage.documentMessage) {
                    await sock.sendMessage(chatId, { text: '📥 Document downloaded successfully.' }, { quoted: message });
                }
                
            } catch (error) {
                console.error('Download error:', error);
                await sock.sendMessage(chatId, { text: '❌ Error downloading media.' }, { quoted: message });
            }
        }
    },
