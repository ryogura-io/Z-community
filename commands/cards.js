        ...
        execute: async ({ sender, chatId, args, bot }) => {
            try {
                ...
                try {
                    ...
                    <body>
                        <div class="deck-title">${player.name}'s Deck (${deckCards.length}/12 cards)</div>
                            ...
                            ${Array.from({ length: 12 }, (_, i) => {
                                ...
                                if (card) {
                                    return `<div class="card-slot">
                                        ...
                                        <div class="card-info">${card.name} (T${card.tier})</div>
                                    </div>`;
                                } else {
                                    return `<div class="card-slot empty-slot">Empty Slot</div>`;
                                }
                            }).join("")}
                        </div>
                    </body>
                    ...
                    return bot.sendImage(chatId, screenshot, caption);
                } catch (screenshotError) {
                    console.error("Screenshot error:", screenshotError);

                    // Fallback to text-based deck display
                    let deckMsg = `🃏 *${player.name}'s Deck (${deckCards.length} cards)*\n\n`;
