...
class Bot {
    ...
    async processMessage(message) {
        try {
            ...
            await this.addCommandExp(sender, commandName);
        ...
        try {
            ...
            for (const participant of participants) {
                if (action === 'add') {
                    ...
                    await this.sendMessage(groupId, welcomeMsg);
                } else if (action === 'remove') {
                    await this.sendMessage(groupId, `👋 Goodbye @${participant.split('@')[0]}! Thanks for being part of our community.`);
                }
            }
        } catch (error) {
    ...
    async addCommandExp(userId, commandName) {
        try {
            const Player = require('./models/Player');
            ...
            if (player) {
                ...
                if (newLevel > player.level) {
                    ...
                    player.shards += newLevel * 100; // Level up bonus
                }
